...
- **Configuration:** The singleton `MicrosoftIntegrationConfiguration` stores tenant/client identifiers and the DPAPI-protected shared client secret, with credential revision and migration/reset metadata. `MicrosoftLoginConfiguration` stores the SSO switch, default-off `AllowAutoRegistration` flag, health/origin metadata, and audit timestamps; the credential secret is never duplicated there. `SharePointConfiguration.UseSharedMicrosoftCredentials` selects the shared source per existing destination; false preserves independent legacy SharePoint credentials. Secrets are never returned to views, stored in `appsettings.json`, or logged.
- **Identity:** `ApplicationUser` stores verified Microsoft tenant/object mapping and link metadata. `MicrosoftAccountLinkIntent` is short-lived, single-use, server-side state bound to UserId, security stamp, browser binding, and credential revision. Existing users must link explicitly; optional JIT registration is available only when an Admin enables it and creates passwordless Member accounts from validated tenant identities. Email-only linking, group role mapping, and implicit identity moves are never allowed.
...
| `/Dashboard` | Authenticated | Admin/global or Moderator/owned operational analytics; Member personal monthly spending and recent bills. Admin and Moderator can switch between their operational view and their personal spending view via `?view=operations\|personal` (defaults to operational). Members always see the personal dashboard regardless of the query string. |
...
Admin and Moderator users can switch to a **personal spending view** (`?view=personal`) via the segmented selector above the dashboard heading, which shows all non-Draft bills linked to their `ParticipantAccountLink` — including bills uploaded by other users, and bills from before any role change. The selector has two options: Admin sees "Semua transaksi / All transactions" and "Pengeluaran saya / My spending"; Moderator sees "Transaksi saya kelola / Transactions I manage" and "Pengeluaran saya / My spending". Switching is a bookmarkable `GET` request; no database field, cookie, or role change is introduced. The two perspectives are never mixed into one total. Currency rate and trend cards are shown only in the operational view.
