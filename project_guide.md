...
Guest participants remain name-only. `GuestAccessLink` stores a SHA-256 token hash plus DPAPI-protected ciphertext for authorized recopy. One active token opens `/g/my-bill/{token}` and `/g/transaction/{token}`; both are GET-only, read-only projections. Receipt images are token-scoped and protected by the authorized controller. Guest pages intentionally omit account identifiers, payment proofs, approval notes, secrets, and management controls. The uploader/Admin can copy, regenerate, or revoke a guest link through authenticated antiforgery POSTs. Copy uses the Clipboard API when available, a legacy copy command on private HTTP origins, and a selected read-only URL field if automatic copying is blocked; a token enters the DOM only after an authorized copy request.
