...
Guests are entered by name only. After Step 3, every participant without a registered account can have one active participant-scoped bearer link. The uploader or Admin opens the Share icon beside the transaction status in Transaction Details to manage links; controls are no longer embedded in payment rows. Registered accounts do not receive guest links.

The participant URL opens `/g/my-bill/{token}` and retains a switch to `/g/transaction/{token}` using the same token. My Bill shows that guest's allocation; Whole Transaction shows participant display names, orders, amounts, statuses, receipt details, pickup result, and currency snapshot. The same token keeps the historical whole-transaction URL working. A separate transaction-wide token is created on demand from the modal's transaction link row. It opens only `/g/transaction/{token}` and has no My Bill tab or participant identity. Both token types are read-only and hide usernames, emails, account IDs, payment proofs, approval notes, and management actions.

On HTTPS or localhost, the copy button uses the browser Clipboard API. On an HTTP private-network address, where that API may be unavailable, it tries the browser's legacy copy command. If the browser blocks both, the requested URL appears in a selected read-only field for manual copying. That field is created only after the authorized POST succeeds; the guest token is never embedded in the initial page HTML. Regenerate revokes the old token before issuing a new one; Revoke affects only the selected transaction or participant token. A server-side link failure shows a separate message. Admins can manage links on every transaction, while Moderators can manage only their own uploaded transactions; Admins can request the JPEG share projection/export for every saved transaction, while Moderators can request it only for their own uploaded transactions.
